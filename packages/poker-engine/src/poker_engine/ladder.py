...
обязано давать одно сообщение. Порядок собственных гардов класса тоже
повторяет `_validate_context`: пустота, затем интервалы, затем
`places_paid` и глубина лесенки. Приоритет сообщений валидатора целиком
этим не сохраняется: монотонность призов он проверяет внутри цикла по
интервалам, а `entrants < players_left` — между циклом и `places_paid`,
и ни того, ни другого класс не делает. Где встанут эти две проверки,
когда Задача 4 переведёт ладдер-блок валидатора на этот класс, решает
она (долг D17 документа фаз). Пока живут две модели места — здешняя
