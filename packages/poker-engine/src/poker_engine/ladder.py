...
пакет (`TournamentContext.ladder()` из `payouts`, где `handstate` уже
привёл поля к числам), поэтому строка не той длины или приз строкой
...
Тексты отказов взяты дословно из прежнего `handstate._validate_context`:
там те же нарушения установили пользовательский контракт, и одно
нарушение обязано давать одно сообщение. Теперь валидатор строит этот
класс сам, и ладдер-гарды живут в одной реализации. Обещание приоритета
относится только к гардам класса: пустота, затем интервалы по порядку
(границы, приз, пересечение), затем `places_paid` и глубина лесенки.

Монотонность призов к худшим местам класс не проверяет: расчёту по
лесенке она безразлична, и проверка остаётся у `handstate`, после
конструктора — как и `entrants < players_left` (решение Задачи 4 плана
`docs/superpowers/plans/2026-09-12-icm-field-model.md`, долг D17).
"""
...
class PayoutLadder:
    def __init__(
        ...
        # лобби не платит за место, которое не оплачивается. Принять его
        # значит считать ICM с призами, которых турнир не выдаёт, а
        # `is_complete` при этом соврал бы о полноте.
