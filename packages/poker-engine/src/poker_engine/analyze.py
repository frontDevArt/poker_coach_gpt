...
def analyze(
    ...
    # Неопределённое давление (деньги вне достижимых мест, лесенка,
    # награждающая вылет) — не ошибка ввода, а свойство лесенки: сообщаем
    # пометкой ниже, а не падением всего разбора. Winner-take-all сюда не
    # относится: на модели поля его давление определено — риск-премия 0,
    # bubble factor 1.0 (`icm_field.risk_premium`). Ловится только
