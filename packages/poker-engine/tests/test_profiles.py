...
def test_range_is_minimal_not_merely_covering():
    ...
    #
    # Инвариант двусторонний, и обе стороны проверяются здесь: цель
    # покрыта, но без последнего класса не была бы.
    #
    # Вложенность следует из конструкции (и проверена выше на одной паре),
    # поэтому множество возвращаемых размеров — это в точности кумулятивные
    # суммы префиксов списка классов. Значит предыдущий размер можно взять
    # из самих ответов функции, не повторяя правило отбора в тесте.
