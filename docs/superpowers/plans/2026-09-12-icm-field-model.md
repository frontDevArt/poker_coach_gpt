...
**Решения по находкам ревью (Фаза 9, 2026-09-25).** Долг D21, решение пользователя
2026-09-25: оценщик `eval7` заменён на `phevaluator 0.6.0` — у `eval7 0.1.11` нет колёс для
Windows на Python 3.13+ и нет sdist. Против текста задачи:

1. Везде, где шаги и приёмка называют `eval7`, читать `phevaluator`. Приёмка п.1 — тест
   согласия `phevaluator` с `pokerkit`, оба теста `test_equity_evaluator.py` (3000 случайных
   пар и 3000 пар на общей доске с ничьими). П.5 — `phevaluator>=0.6.0` в `pyproject.toml`,
   `phevaluator==0.6.0` в локе; `eval7`, `future`, `pyparsing` из лока убраны, фильтр
   `DeprecationWarning` на `eval7.rangestring` снят (у `phevaluator` нет зависимостей).
2. Шкала `phevaluator` обратная: меньший ранг — сильнее. `_score_runout` берёт `min`, тест
   согласия сравнивает `-evaluate_cards(...)`. Мутант `max` вместо `min` убит.
3. Карты передаются `evaluate_cards` номерами (`Card(code).id_`, один раз на модуль), а не
   строками: строки замедляют оценку вчетверо (0.30 с против 0.08 с на 100 000 рук). Разбор
   эталонной фикстуры — 0.16 с при бюджете 2 с; с `equity.py` из `a42dfee` тест бюджета
   краснеет (8.02 с).
4. `test_equity_crosscheck.py` не правился.

---

## Финал

- [x] Финальное ревью всей ветки против спеки целиком, тем же циклом «ревью → починка →
      ревью» до `НАХОДОК НЕТ`.
- [x] Журнал в `docs/superpowers/state/2026-09-12-icm-field-execution-notes.md`:
      что решено против текста плана, какие тесты удалены и почему, замеры до и после,
      что осталось отложенным.
- [x] Обновить `CLAUDE.md`: текущее состояние, таблица документов, точка входа.
- [x] Открытые вопросы спеки §11, не закрытые планом, перенести в журнал списком.

Сделано Фазами 8–10 (2026-09-25): итог — раздел «Итог плана 3» журнала
`docs/superpowers/state/2026-09-12-icm-field-execution-notes.md`.
