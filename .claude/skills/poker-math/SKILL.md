...
- `--bounty` — число, показанное над игроком на экране; оно и есть наличные за нокаут. Не делить пополам: половиной полного баунти оно уже является (внёс $3.00 — показано $1.50). Флага `--split` больше нет.
...
**PKO** — голова засчитывается, только если колл героя покрывает стек соперника. Иначе `bounty-ev` возвращает обычный пот-оддс, и это не ошибка. Засчитывается показанный ценник целиком: выбивший получает его наличными, а его собственный ценник растёт на половину взятого ($1.50 → $2.25).

## Известные ограничения — упоминать в разборе

- `icm` и `risk-premium` считают только переданные стеки, без поля: для них `--stacks` — весь турнир. Числа верны, только когда за столом весь оставшийся турнир (финальный стол). В середине турнира, когда живых больше, чем мест за столом, ICM-числа брать из `analyze` (модель «стол + поле»); `icm`/`risk-premium` на столе посреди турнира считают его целым турниром, и давление расходится в разы.
...
- Прирост собственного ценника после нокаута (половина взятого) в EV раздачи не моделируется — считаются только наличные, то есть показанный ценник соперника.
