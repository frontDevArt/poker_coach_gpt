...
Для 9, 8, 7, 5, 4 и 3 игроков список — это хвост 9-max порядка нужной длины (отрезаны только самые ранние позиции); 6-max и хедз-ап именуются по отдельному соглашению и хвостом не получаются. Источник истины — `poker_engine.types.positions_for(n)`.
